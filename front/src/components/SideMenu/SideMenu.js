        ...
        <ul className="inner-second" style={{ maxHeight: '571px' }}>
          {Side?.find((m) => m.id == activeMarket)?.Categories?.map((category) => {
            return (
              <li
                key={category.id}
                className={`menu-item has-children third-level-container ${category.id === activeCategory ? 'active' : ''}`}
              >
                <a
                    ...
                    <ul className="inner-third" style={{ maxHeight: '571px' }}>
                      {category.Subcategories.length > 0 ? (
                        category.Subcategories.map((subcategory) => (
                          <li
                            key={subcategory.id}
                            className={`menu-item ${subcategory.id === activeSub ? 'active' : ''}`}
                          >
                            <a
                            ...
                            ></i>
                            {showFourthLevel && subcategory.id === activeSub && (
                              <ul
                                className="fourth-level menu-products-container"
                                style={{ display: showFourthLevel ? 'block' : 'none' }}
                              >
                                {Side.find((m) => m.id == activeMarket)
                                  ?.Categories.find((c) => c.id == activeCategory)
                                  ?.Subcategories.find((c) => c.id == activeSub)
